...
const SYMBOL_PATTERNS = {
  ...
  js: {
    ...
    // Classes
    class: [/class\s+(\w+)(?:\s+extends\s+(\w+(?:\.\w+)*))?/g],
    // Interfaces
    interface: [/interface\s+(\w+)(?:\s+extends\s+(\w+(?:\.\w+)*))?/g],
    // Variables (let, const, var)
    variable: [/(?:let|const|var)\s+(\w+)\s*=/g],
    // Methods within classes
    method: [/(?:async\s+)?(\w+)\s*\([^)]*\)\s*{/g],
    // Imports
    imports: [/import\s+(?:{\s*([^}]*)\s*}|([^{}\s]+))\s+from\s+['"]([^'"]+)['"]/g],
  },
};
 ...
 */
function extractCalls(filePath: string, repositoryPath: string, symbols: Record<string, CodeSymbol>): CallRelation[] {
...
function extractInheritance(symbols: Record<string, CodeSymbol>): InheritanceRelation[] {
  ...
  for (const symbol of Object.values(symbols)) {
    if ((symbol.type === SymbolType.Class || symbol.type === SymbolType.Interface) && symbol.properties?.extends) {
      relations.push({
        child: symbol.name,
    ...
    if (symbol.type === SymbolType.Class && symbol.properties?.implements) {
