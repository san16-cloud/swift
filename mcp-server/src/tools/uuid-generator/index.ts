...
export class UuidGeneratorTool extends BaseTool<UuidGeneratorInput, UuidGeneratorOutput> {
  ...
  protected getSchema(): Record<string, z.ZodType<unknown>> {
    return {
      version: z.enum(['v1', 'v4', 'v5']).optional().default('v4').describe('UUID version to generate'),
      namespace: z.string().optional().describe('Namespace for v5 UUIDs'),
      count: z.number().int().min(1).max(100).optional().default(1).describe('Number of UUIDs to generate (1-100)'),
      format: z
   ...
   */
  protected formatResponse(result: UuidGeneratorOutput): ToolResponse {
    return {
      content: [
        {
          type: 'text',
