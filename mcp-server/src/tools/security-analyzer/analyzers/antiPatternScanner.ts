  ...
  javascript: [
    {
      ...
      patterns: [/cors\(\{\s*origin\s*:\s*(?:['"]?\*['"]?|\[['"]?\*['"]?\])/i, /Access-Control-Allow-Origin:\s*\*/i],
    ...
    },
    {
      ...
      patterns: [/\beval\s*\(/i, /new\s+Function\s*\(/i],
  ...
  node: [
    {
      ...
      patterns: [/app\s*=\s*express\(\)(?:\s*;)?(?:[\s\S]{0,500}?)(?!app\.use\(helmet)/i],
    ...
    },
    {
      ...
      patterns: [/(?:===|!==|==|!=)\s*(?:password|token|secret)/i],
...
function getFileExtensions(framework?: string): string[] {
  if (!framework) {
    ...
    return ['.js', '.jsx', '.ts', '.tsx', '.py', '.php', '.rb', '.java', '.go', '.cs'];
  }
