  ...
  async store(summaryData: Record<string, unknown>, detailedData?: Record<string, unknown>): Promise<StorageResult> {
    ...
    try {
      ...
      if (!this.repositoryInfo.path) {
        logInfo(`Skipping analytics storage for repository-less tool ${this.toolId}`, this.SERVICE_NAME, this.SERVICE_VERSION);
        return {
      ...
      if (!repoBasePath) {
        logInfo(`Skipping analytics storage for ${this.toolId} due to invalid repository path`, this.SERVICE_NAME, this.SERVICE_VERSION);
        return {
          snapshotId: 'analytics-skipped',
          snapshotPath: '',
          metadata: this.collectMetadata()
