...
export interface DependencyAnalysisResult {
  ...
  dependencyCounts: Record<string, {    // Count of dependencies for each file
    ...
    for (const pattern of patterns) {
      ...
      while ((match = pattern.exec(content)) !== null) {
        if (match[1]) {
          ...
          const isExternal = !targetModule.startsWith('.') && 
          ...
          dependencies.push({
            source: relativePath,
            target: targetModule,
            type: pattern.toString().includes('require') ? 'require' : 
                  pattern.toString().includes('include') ? 'include' : 'import',
            isExternal
          });
        }
      }
